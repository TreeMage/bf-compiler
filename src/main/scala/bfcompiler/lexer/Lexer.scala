...
object Lexer {
  val default: Lexer = new Lexer:
    ...
    def lexLine(line: String, lineIndex: Int, filePath: Path): LexerResult =
      val parsed = line.toList.zipWithIndex.map {
        case (value, column) =>
          ...
          value match
            ...
            case ']' => Right(common.Token(Lexeme.JumpBackwardEqualZero, location))
    ...
    override def lexFile(path: Path): LexerResult =
      val lexed =
      for
        source <- Try(scala.io.Source.fromFile(path.toFile)).toEither
      yield
        source.getLines().toList.zipWithIndex.map {
          case (line, index) => lexLine(line, index, path)
        }
      lexed.fold(
        throwable => LexerResult.fromError(LexerError.IOError(path, throwable)),
        results => results.sequence match
