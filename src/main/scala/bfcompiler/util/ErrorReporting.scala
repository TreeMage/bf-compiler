...
object ErrorReporting {
  ...
  private def reportErrorsGeneric[A](errors: List[A], header: String = "")(formatter: A => String): Unit =
    if (!header.isBlank)
  ...
  def reportLexerErrors(errors: List[LexerError]): Unit =
    reportErrorsGeneric(errors, "Lexing failed due to the following issue(s):") {
  ...
  def reportIntermediateCompilationErrors(errors: List[IntermediateCompilationError]): Unit =
    reportErrorsGeneric(errors, "Intermediate code generation failed due to the following issue(s):") {
